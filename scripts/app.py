...
def home():
    ...
    with report_col1:
        st.markdown(f"""
                ...
                **MRI Result:** 
                    ...
                    -  Confidence: {st.session_state.mri_tumor_probability:.2f}%
    ...
    st.info(f"🆔 Feedback ID: {st.session_state.feedback_id}")
    ...
    if st.session_state.report_submitted:
        st.download_button(
            ...
            data=save_text_report(st.session_state.feedback_id, ct_tumor_result, mri_tumor_class, report.name,
                                  report.phone,
                                  report.email, report.comments),
